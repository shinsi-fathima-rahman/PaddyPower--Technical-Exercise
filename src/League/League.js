...
const League = () => {
    ...
    const fetchTeamPlayers = async (id) => {
        try {
            const response = await fetch(`https://soccer.sportmonks.com/api/v2.0/teams/${id}?api_token=HOLCAStI6Z0OfdoPbjdSg5b41Q17w2W5P4WuoIBdC66Z54kUEvGWPIe33UYC&include=squad`);
            return await response.json();
        }
        catch (err) {
            console.log(err);
    ...
    return (
        <>
            <div className="league__main">
                {error.length > 0 ? <div className="league__error">{error}</div> : <div></div>}
